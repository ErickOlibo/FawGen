...
//
//  A simplified and changed version of the Gunay Mert Karadogan version
//  All credits for the inspiration comes from Gunay Mert Karadogan
//  [GMStepper.swift] - Created by Gunay Mert Karadogan on 1/7/15.
//  Copyright © 2015 Gunay Mert Karadogan. All rights reserved.
//  From Github User: https://github.com/gmertk/GMStepper

import UIKit


public class TEOStepper: UIControl {
    ...
    /// Provide a state to know if the values are text like
    /// or integer and double like
    /// - Author: Erick Olibo
    /// - Note: Maybe push this solution to the community
    ...
    /// Extra info added by Erick Olibo
    @objc public var value: Double = 0 {
        didSet {
            value = min(maximumValue, max(minimumValue, value))
            
            // changed line
            label.text = isCollection ? textToDisplay() : formattedValue
    ...
    @objc public var minimumValue: Double = 0 {
        didSet {
            value = min(maximumValue, max(minimumValue, value))
        }
    }
    ...
    @objc public var maximumValue: Double = 100 {
        didSet {
            value = min(maximumValue, max(minimumValue, value))
        }
    }
    ...
    @objc public var stepValue: Double = 1 {
        didSet {
    ...
    @objc public var autorepeat: Bool = true
    
    /// If the value is integer, it is shown without floating point.
    @objc public var showIntegerIfDoubleIsInteger: Bool = true {
        didSet {
            setupNumberFormatter()
        }
    }
    ...
    @objc public var leftButtonText: String = "−" {
        didSet {
    ...
    @objc public var rightButtonText: String = "+" {
        didSet {
    ...
    @objc public var buttonsTextColor: UIColor = UIColor.white {
        didSet {
    ...
    @objc public var buttonsBackgroundColor: UIColor = UIColor(red:0.21, green:0.5, blue:0.74, alpha:1) {
        didSet {
            for button in [leftButton, rightButton] {
    ...
    @objc public var labelTextColor: UIColor = UIColor.white {
        didSet {
    ...
    @objc public var labelBackgroundColor: UIColor = UIColor(red:0.26, green:0.6, blue:0.87, alpha:1) {
        didSet {
    ...
    @objc public var labelCornerRadius: CGFloat = 0 {
        didSet {
    ...
    @objc public var cornerRadius: CGFloat = 4.0 {
        didSet {
    ...
    @objc public var labelWidthWeight: CGFloat = 0.5 {
        didSet {
    ...
    @objc public var limitHitAnimationColor: UIColor = UIColor(red:0.26, green:0.6, blue:0.87, alpha:1)
    ...
    let limitHitAnimationDuration = TimeInterval(0.2)
    ...
    lazy var label: EdgeInsetLabel = {
        let label = EdgeInsetLabel()
        label.textAlignment = .center
        label.leftTextInset = 5
        label.rightTextInset = 5
        
        // line changed by Erick Olibo
        label.text = isCollection ? textToDisplay() : formattedValue
    ...
    fileprivate func setup() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(reset), name: UIApplication.willResignActiveNotification, object: nil)
    }
...
extension TEOStepper {
    ...
    func scheduleTimer() {
        timer = Timer.scheduledTimer(timeInterval: timerInterval, target: self, selector: #selector(handleTimerFire), userInfo: nil, repeats: true)
    }
...
// Created by Erick Olibo
extension TEOStepper {
    
    /// Current label Text as continuation of the Value
    public func textToDisplay() -> String  {
        let text = String()
        guard isCollection else { return text }
        guard let display = textCollection[value] else { return text }
        return display
    }
}
